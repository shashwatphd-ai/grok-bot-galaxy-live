...
- **The true Day 1 story, from speech:** the trio (Lauren, Roshan, Matt Palmer — intros at 0:00) opened with NO idea, announced a viewer contest (Starship-launch trip, ends Sep 29), chose the company through agent dialogue ~10:15–10:50 AM: **a restaurant pop-up — run a real SF pop-up with a partnering chef and venue, ticket it, and productize the ops ("Pop-up OS")**. Guest **Cody Sanchez** joined ~11:25 AM with distribution advice (sell to three first; controversy marketing — 2.4M and 5.7M-view examples; proof vault) and helped create a marketing bot live.
- **Why earlier reads drifted:** keyword counts over the whole transcript — restaurant: 67, venue: 46, event: 59, food: 28; cabin: 0, glider: 0. The "Pine Cabins"/"glider flights"/"family bookings" screens in the PM sessions were the session presenters' demo data (a sample venue company), which I had mistaken for the trio's company. The trio's real artifacts: domain + staging lander, "no PRs — ship to main" rule, Tater (eng bot) + a reviewer bot, night-market ticket stubs, badges/lanyards, Grok Bot plush designs, Tee Bot emails.
...
**[9:10 AM PT] — SE session: whiteboarded customer demo arc.** "Grok Bot Template → Draft your team → Set your lineup → Compete!" 25.3K views.

**[9:35 AM PT] — Vision Deck build.** Amrita's "Why SpaceXAI?" pitch slide assembled and delivered live in Google Slides. 37.4K views.

**[10:06 AM PT] — Chief of Staff bot runs the staffing pipeline.** Host-finder people list, onboarding routed through dr. eggbot, "Weekly priority board" routine scheduled Wednesdays 8:00 AM. Knowledge Base Manager threads active. 63K views.
...
**[4:03–4:50 PM PT] — Customer Support session (David Gan): the four-bot support team, demoed end to end.** Bots: build (infrastructure), reply (answers tickets in Plain and Slack), alert (urgent flags to Slack), tune (improves the system). Stack: Plain ticketing + Notion knowledge base (public docs + internal refund SOP) + Supabase + Stripe, on the Flylo demo ($20/month in-flight wifi). Live: password reset answered citing the exact doc; SSO/Okta → no docs → low confidence → handoff to human + Slack alert; refund tickets — granted Carter's (inside 14 days), denied Damon's (outside) without leaking the SOP. Crawl-walk-run: start read-only, add drafts, then let it answer.

**[~5:04 PM PT] — 🏁 DAY 2 WRAPPED (early sign-off).** Final views ~374.9K+ (crossed 250K during the interview). The broadcast ended about an hour ahead of the nominal 6 PM slot.
